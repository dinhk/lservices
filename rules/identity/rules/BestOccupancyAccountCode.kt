...
class BestOccupancyAccountCode : Rule<Identity, Calculated>() {
    override fun execute(source: Identity, calculations: Calculated): String? {
        ...
        returns(calculations::accountCode)

        if (bestCurrentOccupancy != null)
        {
            return bestCurrentOccupancy.accountCodes?.maxByOrNull { it.percent }?.accountCode
        }
